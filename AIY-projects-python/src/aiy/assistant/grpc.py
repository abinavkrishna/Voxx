...
DEFAULT_GRPC_DEADLINE = 60 * 3 + 5 ############# Changed this from 60 * 3 + 5
...
class AssistantServiceClient:
    ...
    def _assist(self, recorder, play, deadline):
        continue_conversation = False # New variable
        ...
        for response in self._assistant.Assist(self._requests(recorder), deadline):
            if response.event_type == END_OF_UTTERANCE:
                ...
                #logger.info('Inside END_OF_UTTERANCE. "%s"',self._user_text)
                recorder.done()
            ...
            if response.speech_results:
                ...
                if conversation_state == False:
                    logger.info('Text for matching is "%s".',' '.join(r.transcript for r in response.speech_results))
                self._user_text = ' '.join(r.transcript for r in response.speech_results)
            # Process 'audio_out'.
            # Commenting below portion to remove speech response
            """if response.audio_out.audio_data:
                recorder.done()  # Just in case.
                #logger.info(response.audio_out.audio_data)
...
class AssistantServiceClientWithLed(AssistantServiceClient):
    ...
    def __init__(self, board, language_code='en-US', volume_percentage=100):
        ...
        #self._led_controller = LEDController(LED_COUNT, LED_PIN, LED_FREQ_HZ, LED_DMA,LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL,LED_STRIP)
        #self._board = board
    ...
    def _playing_started(self):
        ...
        #self._led_controller.breathing_effect()

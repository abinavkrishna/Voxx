...
def say(text, lang='en-US', volume=60, pitch=130, speed=100, device='default'):
    ...
    with tempfile.NamedTemporaryFile(suffix='.wav', dir=RUN_DIR) as f:
       ...
       global thinking
       global responding
       global response_available
       tts_cmd = 'pico2wave --wave %s --lang %s "%s"' % (f.name,lang,data)
       subprocess.check_call(tts_cmd, shell=True)
       thinking = 0
       responding = 1
       print("Audio ready!")
       response_available.set()
       speech_to_audio_cmd = 'aplay -q -D %s %s' % (device,f.name)
       subprocess.check_call(speech_to_audio_cmd, shell=True)
